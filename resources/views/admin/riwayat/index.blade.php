...
@section('content')
<div id="pageRoot" class="space-y-6">
    {{-- Flash/Error --}}
    @if ($errors->any())
    ...
    @php
        ...
        if (!function_exists('blood_pill')) {
            function blood_pill($g) {
                return '<span class="inline-flex h-6 items-center rounded-full border border-rose-100 bg-rose-50 px-2 text-xs text-rose-700 font-semibold">'.e($g).'</span>';
            }
        }
        if (!function_exists('rhesus_pill')) {
            function rhesus_pill($r) {
                return '<span class="inline-flex h-6 items-center rounded-full border border-slate-100 bg-slate-50 px-2 text-xs text-slate-700 font-semibold">'.e($r).'</span>';
            }
        }
        if (!function_exists('product_pill')) {
            function product_pill($p) {
                return '<span class="inline-flex h-6 items-center rounded-full border border-sky-100 bg-sky-50 px-2 text-xs text-sky-700 font-semibold">'.e($p).'</span>';
            }
        }
    ...
    <form id="filterForm" method="GET" class="space-y-3">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="relative">
                            ...
                            <button type="button" id="produkBtn"
                                ...
                                <svg class="size-4 text-neutral-500 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M6 9l6 6 6-6"/></svg>
                            </button>
                            <div id="produkMenu" role="menu" aria-labelledby="produkBtn"
                                ...
                                @foreach ($produkOpts as $val => $label)
                                    <button type="button" role="menuitemradio"
                                            ...
                                            class="produk-item group w-full text-left rounded-xl px-3 py-2 text-sm transition flex items-center justify-between {{ $produkQ === $val ? 'bg-blue-50 text-blue-800 ring-1 ring-blue-200 cursor-default' : 'text-blue-800 hover:bg-blue-50/60' }}"
                                            data-value="{{ $val }}">
                                        <span class="truncate">{{ $label }}</span>
                                        <svg class="check-icon size-4 opacity-0 group-[aria-checked=true]:opacity-100" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                    </button>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="relative">
                            ...
                            <button type="button" id="golBtn"
                                    class="mt-1 w-full rounded-xl border border-neutral-200 bg-white px-3 py-2 text-sm text-neutral-800 shadow-sm hover:bg-emerald-50/40 focus:outline-none focus:ring-2 focus:ring-navy-200 transition flex items-center justify-between"
                            ...
                            <div id="golMenu" role="menu" aria-labelledby="golBtn"
                                ...
                                @foreach ($gOpts as $val => $lab)
                                    <button type="button" role="menuitemradio"
                                            ...
                                            class="gol-item group w-full text-left rounded-xl px-3 py-2 text-sm transition flex items-center justify-between {{ $golQ === $val ? 'bg-blue-50 text-blue-800 ring-1 ring-blue-200 cursor-default' : 'text-blue-800 hover:bg-blue-50/60' }}"
                                            data-value="{{ $val }}">
                        ...
                        <div class="flex items-center justify-between pt-1">
                            ...
                            <button type="button" id="applyBtn" class="px-3 py-1.5 rounded-lg bg-blue-600 text-white text-xs sm:text-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-200">Terapkan</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="relative sm:ml-auto">
                <button type="button" id="pageSizeBtn"
                    ...
                    <svg class="size-5 text-neutral-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.6" d="M4 8h16M4 16h10"/></svg>
                    <span>Baris: <strong class="font-semibold text-neutral-800">{{ $perPage }}</strong></span>
                    <svg class="size-4 text-neutral-500" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M10 12a1 1 0 0 1-.7-.29l-4-4a1 1 0 0 1 1.4-1.42L10 9.59l3.3-3.3a1 1 0 1 1 1.4 1.42l-4 4A1 1 0 0 1 10 12Z" clip-rule="evenodd"/></svg>
                </button>
        ...
        @if ($hasActiveFilter)
            <div class="flex flex-wrap items-center gap-2">
                @if ($q)
                    <span class="inline-flex items-center gap-1 rounded-full border border-blue-200 bg-blue-50 px-2.5 py-1 text-xs text-neutral-800">
                        Cari: <strong class="font-medium">{{ $q }}</strong>
                        <button type="button" class="ml-1 remove-chip" data-target="q">×</button>
                    </span>
                @endif
                @if ($produkQ !== '')
                    <span class="inline-flex items-center gap-1 rounded-full border border-blue-200 bg-blue-50 px-2.5 py-1 text-xs text-neutral-800">
    ...
    {{-- =================== CARDS (mobile) =================== --}}
    <div id="cardsContainer" class="space-y-3 md:hidden"><!-- diisi via JS --></div>

    {{-- Pagination (client-side) --}}
    <div class="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div id="pageInfo" class="text-sm text-neutral-600">Tidak ada data</div>
...
<div id="detailModal"
    ...
    <div id="detailCard"
        ...
        <div class="sticky top-0 z-10 flex items-center justify-between gap-4 border-b border-neutral-100/80 px-4 py-3 sm:px-6 sm:py-4 bg-white/80 backdrop-blur">
            <div class="min-w-0">
                <h3 id="detailModalTitle" class="truncate text-lg sm:text-xl font-semibold tracking-tight text-neutral-900">Detail Pemesanan</h3>
            ...
            <div class="flex items-center gap-2">
                ...
                <button type="button" class="dm-close inline-flex items-center justify-center rounded-xl p-2 text-neutral-500 ring-1 ring-transparent transition hover:bg-neutral-100 focus:outline-none focus:ring-navy-300" aria-label="Tutup modal">
                    <svg class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.6" d="M6 18 18 6M6 6l12 12"/></svg>
                </button>
            </div>
        </div>
        ...
        <div class="px-4 py-4 sm:px-6 sm:py-5">
            <div class="grid max-h-[70vh] grid-cols-1 gap-4 sm:gap-6 overflow-auto pr-1 md:grid-cols-2">
                ...
                <section class="rounded-2xl border border-neutral-100 bg-neutral-50/60 p-3 sm:p-4">
                    <div class="mb-3 flex items-center gap-2">
                        <span class="inline-flex size-6 items-center justify-center rounded-lg border border-neutral-200 bg-white">
                            <svg class="size-3.5 text-neutral-700" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.6" d="M16 7a4 4 0 1 1-8 0 4 4 0 0 1 8 0M12 14c-4.418 0-8 2.239-8 5v1h16v-1c0-2.761-3.582-5-8-5z"/></svg>
                        </span>
                ...
                <section class="rounded-2xl border border-neutral-100 bg-neutral-50/60 p-3 sm:p-4">
                    <div class="mb-3 flex items-center gap-2">
                        <span class="inline-flex size-6 items-center justify-center rounded-lg border border-neutral-200 bg-white">
                            <svg class="size-3.5 text-neutral-700" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.6" d="M12 8v8M8 12h8M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0z"/></svg>
                        </span>
                ...
                <section class="rounded-2xl border border-neutral-100 bg-neutral-50/60 p-3 sm:p-4 md:col-span-2">
                    <div class="mb-3 flex items-center gap-2">
                        <span class="inline-flex size-6 items-center justify-center rounded-lg border border-neutral-200 bg-white">
                            <svg class="size-3.5 text-neutral-700" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.6" d="M12 21s-6-4.35-6-10a6 6 0 1 1 12 0c0 5.65-6 10-6 10z"/></svg>
                        </span>
                    ...
                    <dl class="two-col-dl">
                        ...
                        <dt>Cek Transfusi</dt><dd id="dm_cek">-</dd>
                    </dl>
        ...
        {{-- Footer --}}
        <div class="sticky bottom-0 z-10 flex items-center justify-end gap-3 border-t border-neutral-100/80 bg-white/80 px-4 py-3 sm:px-6 sm:py-4 backdrop-blur">
...
/* ---------- State ---------- */
let currentPage = Number(@json($perPage)) || 10; // dipakai sebagai pageSize awal dari GET
...
const bloodPill = g => `<span class="inline-flex h-6 items-center rounded-full border border-rose-100 bg-rose-50 px-2 text-xs text-rose-700 font-semibold">${g ?? '-'}</span>`;
const rhesusPill = r => `<span class="inline-flex h-6 items-center rounded-full border border-slate-100 bg-slate-50 px-2 text-xs text-slate-700 font-semibold">${r ?? '-'}</span>`;
const productPill = p => `<span class="inline-flex h-6 items-center rounded-full border border-sky-100 bg-sky-50 px-2 text-xs text-sky-700 font-semibold">${p ?? '-'}</span>`;

const productLabel = c => ({
  WB:'WB: Whole Blood', PRC:'PRC: Packed Red Cell', TC:'TC: Thrombocyte Concentrate',
  FFP:'FFP: Fresh Frozen Plasma', CRYO:'CRYO: Cryoprecipitated Anti-Hemophilic Factor',
  LP:'LP: Liquid Plasma', TCA:'TCA: Thrombocyte Apheresis', CP:'CP: Convalescent Plasma'
})[c] || c || '-';
...
function getPaged(data){
  ...
  return { data: data.slice(start, end), total, pages };
}
...
function renderCards(slice){
  ...
  if(!slice.length){ wrap.innerHTML = `<div class="text-center text-neutral-500">Tidak ada data.</div>`; return; }
...
function renderPagination(total, pages){
  ...
  cont.innerHTML = html;
  cont.querySelectorAll('button[data-page]').forEach(b => b.addEventListener('click', ()=>{ page=+b.dataset.page; masterRender(); }));
}
...
const FOCUSABLE_SELECTOR = [
  ...
  'select:not([disabled])','textarea:not([disabled])','details','[contenteditable="true"]','[tabindex]:not([tabindex="-1"])'
].join(',');
...
function closeModal(overlay, dialog, store){
  ...
  if(dialog.__trapHandler){ dialog.removeEventListener('keydown', dialog.__trapHandler); delete dialog.__trapHandler; }
...
detailCard.addEventListener('keydown', (e)=>{ if(e.key==='Escape'){ e.preventDefault(); closeDetail(); } });
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  function masterRender(){
    ...
    const { data: slice, total, pages } = getPaged(data);
    renderTable(slice);
    renderCards(slice);
  ...
  // search realtime
  if(search){ search.addEventListener('input', ()=>{ page=1; masterRender(); }); }
  ...
  function closeAll(excepts=[]){
    MENUS.forEach(m=>{ if(m && !excepts.includes(m)) close(m); });
  ...
  filterBtn.addEventListener('click',(e)=>{
    ...
    closeAll();
    if(willOpen){ open(filterMenu); setExpanded(filterBtn,true); }
  });
  ...
  produkBtn.addEventListener('click', (e)=>{
    ...
    closeAll([filterMenu]);
    if(willOpen){ open(produkMenu); setExpanded(produkBtn,true); updateProdukActive(produkSelected||''); }
  });
  produkMenu.querySelectorAll('.produk-item').forEach(item=>{
    item.addEventListener('click', ()=>{
      ...
      close(produkMenu); setExpanded(produkBtn,false);
    });
  });
  ...
  golBtn.addEventListener('click', (e)=>{
    ...
    closeAll([filterMenu]);
    if(willOpen){ open(golMenu); setExpanded(golBtn,true); updateGolActive(golSelected||''); }
  });
  golMenu.querySelectorAll('.gol-item').forEach(item=>{
    item.addEventListener('click', ()=>{
      ...
      updateGolActive(val);
      close(golMenu); setExpanded(golBtn,false);
    });
  });
  ...
  reset.addEventListener('click', ()=>{
    produkSelected = ''; golSelected = '';
    produkLabel.textContent='Semua Produk'; golLabel.textContent='Semua';
    produkInput.value=''; golInput.value='';
    const q = form.querySelector('input[name="q"]'); if(q) q.value='';
    closeAll(); form.submit();
  });
  ...
  document.querySelectorAll('.remove-chip').forEach(btn=>{
    btn.addEventListener('click',(e)=>{
      ...
      if(target==='produk'){ produkSelected=''; produkInput.value=''; }
      else if(target==='gol'){ golSelected=''; golInput.value=''; }
      else if(target==='q'){ const q = form.querySelector('input[name="q"]'); if(q) q.value=''; }
      closeAll(); form.submit();
    });
  });
  ...
  if(clearAllBtn) clearAllBtn.addEventListener('click',(e)=>{
    e.preventDefault();
    const q = form.querySelector('input[name="q"]'); if(q) q.value='';
    produkSelected=''; golSelected=''; produkInput.value=''; golInput.value='';
    closeAll(); form.submit();
  });
  ...
  pageSizeBtn.addEventListener('click',(e)=>{
    ...
    closeAll();
    if(willOpen){ open(pageSizeMenu); setExpanded(pageSizeBtn,true); }
  });
  pageSizeMenu.querySelectorAll('button[data-size]').forEach(item=>{
    item.addEventListener('click',()=>{
      perPageInput.value = item.getAttribute('data-size');
      pageSizeMenu.querySelectorAll('button[role="menuitemradio"]').forEach(b=> b.setAttribute('aria-checked', b===item ? 'true' : 'false'));
      closeAll();
  ...
  document.addEventListener('click', (e)=>{
    const safe = e.target.closest('#filterMenu, #pageSizeMenu, #produkMenu, #golMenu, #filterBtn, #pageSizeBtn, #produkBtn, #golBtn');
    if(!safe) closeAll();
  });
  ...
  document.addEventListener('click', (e)=>{
    ...
    if(!b) return;
    try{ openDetail(JSON.parse(b.dataset.payload || '{}')); }
    catch(err){ console.error('Payload invalid:', err); alert('Gagal membuka detail.'); }
  });
});
</script>
...
@media (min-width:768px){
  .two-col-dl{ grid-template-columns:1fr 1.2fr; gap:.5rem 2rem; }
}

/* Animasi modal */
#detailModal.modal-show{ opacity:1; }
#detailModal:not(.modal-show){ opacity:0; }
