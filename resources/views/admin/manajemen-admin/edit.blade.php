...
@section('content')
  <div class="mx-auto max-w-2xl space-y-6">
    ...
    <div>
      ...
      <h1 class="text-2xl font-semibold md:text-3xl">Edit Admin</h1>
      <p class="mt-1 text-sm text-neutral-500">Perbarui informasi akun administrator</p>
    </div>
      ...
      <form action="{{ route('admin.admins.update', $admin) }}"
        ...
        <div class="rounded-xl border border-neutral-200 bg-neutral-50 p-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="font-medium text-neutral-800">{{ $admin->name }}</p>
              <p class="text-xs text-neutral-500">Terdaftar sejak {{ $admin->created_at->format('d M Y') }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="border-t border-neutral-200 pt-5">
          <p class="mb-1 text-sm font-medium text-neutral-700">Ubah Password (Opsional)</p>
          <p class="mb-4 text-xs text-neutral-500">Kosongkan jika tidak ingin mengubah password</p>
        </div>
        ...
        @if ($admin->id === auth('admin')->id())
          <div class="rounded-xl border border-amber-200 bg-amber-50 p-4">
            <div class="flex gap-3">
              <svg class="h-5 w-5 flex-shrink-0 text-amber-600"
                   viewBox="0 0 24 24"
                   fill="none"
                   stroke="currentColor">
                <path stroke-linecap="round"
              ...
              <div class="text-sm text-amber-800">
                <p class="font-medium">Anda sedang mengedit akun Anda sendiri</p>
                <p class="mt-1 text-amber-700">Jika mengubah email atau password, Anda perlu login ulang dengan
                  kredensial baru</p>
              </div>
            </div>
          </div>
