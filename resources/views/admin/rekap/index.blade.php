...
<div class="px-6 pt-6 pb-10">
  ...
  {{-- Cards ringkasan dari tabel rekap_stok --}}
  ...
  <div class="mt-8 flex flex-wrap items-center gap-3">
    ...
    <form method="GET" action="{{ route('admin.stok.index') }}" class="relative">
      <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari ID/Komponen/Keterangan"
      ...
      <tbody class="divide-y divide-slate-100">
        @forelse ($rekap as $row)
          <tr class="odd:bg-white even:bg-slate-50/50">
            ...
            <td class="px-4 py-3 text-center">{{ \Illuminate\Support\Carbon::parse($row->tanggal_masuk)->format('d M Y') }}</td>
            ...
            <td class="px-4 py-3">
              <div class="flex items-center justify-center gap-2">
                <a href="{{ route('admin.stok.edit',$row->id) }}"
                   class="rounded border border-slate-300 px-2 py-1 text-slate-700 hover:bg-slate-100" title="Edit">✏️</a>
                <form method="POST" action="{{ route('admin.stok.destroy',$row->id) }}"
                      onsubmit="return confirm('Hapus data ini?')">
                  @csrf @method('DELETE')
                  <button class="rounded border border-slate-300 px-2 py-1 text-red-600 hover:bg-red-50" title="Hapus">🗑️</button>
                </form>
              </div>
            </td>
          </tr>
        @empty
          <tr>
            <td colspan="8" class="px-4 py-6 text-center text-slate-500">Belum ada data stok darah.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
