          ...
          <div class="space-y-3">
            <div>
              ...
              <select id="statusSelect"
                ...
                <option value=""
                        {{ $statusQ === '' ? 'selected' : '' }}>Semua</option>
                <option value="approved"
                        {{ $statusQ === 'approved' ? 'selected' : '' }}>Approved</option>
                <option value="pending"
                        {{ $statusQ === 'pending' ? 'selected' : '' }}>Pending</option>
                <option value="rejected"
                        {{ $statusQ === 'rejected' ? 'selected' : '' }}>Rejected</option>
              </select>
            </div>
            <div>
      ...
      <div class="flex items-center gap-2 sm:ml-auto">
        ...
        <select id="pageSize"
          ...
          @foreach ([5, 10, 20] as $opt)
            <option value="{{ $opt }}"
                    {{ $perPage === $opt ? 'selected' : '' }}>{{ $opt }}</option>
          @endforeach
        </select>
                ...
                <td class="px-4 py-3">
                  @if ($o->status === 'pending')
                    <div class="flex items-center gap-2">
                      {{-- APPROVE --}}
                      <form method="POST"
                            action="{{ route('admin.verifikasi.store', $o) }}">
                        @csrf
                        <input type="hidden"
                               name="status"
                               value="approved">
                        <button
                                class="rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs text-blue-700 hover:bg-blue-100">
                          Terima
                        </button>
                      </form>
                      {{-- REJECT --}}
                      <form method="POST"
                        ...
                        <input type="hidden"
                               ...
                               value="rejected">
                        <button
                                class="rounded-lg border border-rose-200 bg-rose-50 px-3 py-1.5 text-xs text-rose-700 hover:bg-rose-100">
                          Tolak
                        </button>
                      </form>
                    </div>
                  @elseif($o->status === 'approved')
                    <span
                          class="inline-flex items-center rounded-lg bg-neutral-100 px-3 py-1 text-xs text-neutral-600">Disetujui</span>
                  @else
                    <span
                          class="inline-flex items-center rounded-lg bg-neutral-100 px-3 py-1 text-xs text-neutral-600">Ditolak</span>
                  @endif
                </td>
              </tr>
            @empty
