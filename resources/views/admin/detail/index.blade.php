...
function renderPagination(total,pages){
  const cont=document.getElementById('pagination'); const info=document.getElementById('pageInfo');
  if(!cont||!info) return;
...
function hkRenderTable(data){
  const tb=document.getElementById('hkTableBody'); if(!tb) return;
...
function hkRenderPagination(total,pages){
  const cont=document.getElementById('hkPagination'); const info=document.getElementById('hkPageInfo');
  if(!cont||!info) return;
...
function exGetSorted(data){
  const cp=[...data];
  cp.sort((a,b)=>{ let va=a[exSortKey], vb=b[exSortKey]; va=String(va??'').toLowerCase(); vb=String(vb??'').toLowerCase();
...
function exRenderTable(data){
  const tb=document.getElementById('exTableBody'); if(!tb) return;
  if(!data.length){
...
function exRenderPagination(total,pages){
  const cont=document.getElementById('exPagination'); const info=document.getElementById('exPageInfo');
  if(!cont||!info) return;
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  exReset?.addEventListener('click', ()=>{ ['exGolSelect','exRhesusSelect','exProdukSelect','exMasukFrom','exMasukTo'].forEach(id=>{ const el=document.getElementById(id); if(el) el.value=''; }); exCurrentPage=1; exRenderAll(); });
...
<style>
  ...
  .tabbtn{ padding: .5rem 1rem; font-size:.875rem; border-radius:.75rem; border:1px solid transparent; color:#525252; background:transparent; transition:background-color .15s ease,color .15s ease,border-color .15s ease; }
  .tabbtn.is-active{ background:#171717; color:#fff; border-color:#171717; }
