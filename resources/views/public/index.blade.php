...
@section('title', 'Form Pemesanan – SIMPHONY')
  ...
  <div class="min-h-screen bg-white text-slate-800">
    ...
    <section class="py-10 sm:py-14">
      <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        {{-- CARD --}}
        <div class="relative rounded-3xl border border-slate-200 bg-white shadow-[0_10px_40px_-10px_rgba(0,0,0,0.12)]">
          {{-- header card --}}
          <div class="flex items-center justify-between border-b border-slate-100 p-5 sm:p-6">
            <div class="flex items-center gap-3">
              ...
              <h1 class="text-lg font-bold sm:text-xl">Data Pasien & Rumah Sakit</h1>
            </div>
            {{-- progress ringan --}}
          ...
          {{-- body card / FORM --}}
          <form method="POST"
                action="{{ route('pemesanan.store.step1') }}"
                class="p-5 sm:p-6">
            @csrf
            <div class="grid gap-5 sm:grid-cols-2">
              ...
              <div>
                ...
                <select name="rumah_sakit"
                        required
                        class="mt-2 w-full rounded-xl border-slate-200 bg-slate-50/50 focus:border-red-500 focus:ring-red-500">
                  <option value=""
                          selected
                          disabled>Pilih Rumah Sakit</option>
                  @foreach ($rsOptions ?? ['RSUD A', 'RSUD B', 'RS PMI'] as $rs)
                    <option value="{{ $rs }}"
                            @selected(old('rumah_sakit') === $rs)>{{ $rs }}</option>
                  @endforeach
                </select>
                @error('rumah_sakit')
              ...
              {{-- Status Suami/Istri --}}
              <div>
                <label class="block text-sm font-medium text-slate-700">Nama Suami/Istri <span
                        class="text-red-600">*</span></label>
                <select name="status_perkawinan"
                  ...
                  <option value="Menikah"
                          @selected(old('status_perkawinan') === 'Menikah')>Menikah</option>
                  <option value="Belum Menikah"
                          @selected(old('status_perkawinan') === 'Belum Menikah')>Belum Menikah</option>
                  <option value="Cerai"
                          @selected(old('status_perkawinan') === 'Cerai')>Cerai</option>
                </select>
            ...
            <div class="mt-8 flex justify-center">
              <button type="submit"
                      class="inline-flex items-center justify-center gap-2 rounded-xl bg-red-600 px-6 py-3 text-white shadow hover:bg-red-700">
                Berikutnya
              </button>
            </div>
          </form>
