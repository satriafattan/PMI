...
<head>
  ...
  <style>
    ...
    .header { border-bottom:3px solid #dc2626; padding-bottom:8px; margin-bottom:14px; display:flex; align-items:center; gap:10px; }
    .header img { width:34px; height:34px; }
    .badge { border:2px solid {{ $config['color'] }}; background: {{ $config['bg'] }}; padding:10px 24px; display:inline-block; border-radius:4px; font-weight:700; color: {{ $config['color'] }}; letter-spacing:.6px; }
    table.detail { width:100%; border-collapse:collapse; border:1px solid #d1d5db; margin-top:12px; }
    table.detail th, table.detail td { border:1px solid #d1d5db; padding:8px 10px; font-size:12px; }
    table.detail thead th { background:#f9fafb; text-align:left; }
    .note { font-size:12px; line-height:1.7; text-align:justify; }
    .stamp { position: fixed; right: 80px; bottom: 150px; width: 140px; opacity:.22; }
    .footer { border-top:1px solid #e5e7eb; margin-top:18px; padding-top:12px; font-size:11px; color:#6b7280; display:flex; justify-content:space-between; }
  </style>
</head>
<body>

  <div class="header">
    <img src="{{ public_path('images/pmi-logo.png') }}" alt="PMI">
    <div>
  ...
  <p style="font-size:12px;margin:12px 0 0 0;">Kepada Yth.<br><strong>{{ $nama }}</strong><br>{{ $rsPemesan }}</p>

  <p class="note" style="margin-top:12px;">Dengan hormat,</p>
  <p class="note">Melalui surat ini, kami sampaikan pemberitahuan terkait status permohonan pemesanan darah yang telah Bapak/Ibu ajukan dengan rincian sebagai berikut:</p>
  ...
  <table class="detail">
    <thead>
      <tr><th colspan="2">RINCIAN PEMESANAN</th></tr>
    </thead>
    <tbody>
      <tr><td style="width:40%;">Nomor Pemesanan</td><td><strong>#{{ $pemesanan->id ?? 'N/A' }}</strong></td></tr>
      <tr><td>Produk Darah</td><td>{{ $produk }}</td></tr>
      <tr><td>Golongan Darah</td><td><strong>{{ $gol }}{{ $rhesus ? ' ' . $rhesus : '' }}</strong></td></tr>
      <tr><td>Jumlah Kantong</td><td>{{ $jumlah }} Kantong</td></tr>
      <tr><td>Rumah Sakit Pemesan</td><td>{{ $rsPemesan }}</td></tr>
    </tbody>
  </table>

  @if ($statusKey === 'approved')
    <p class="note" style="margin-top:12px;">
      Berdasarkan hasil verifikasi dan pengecekan ketersediaan stok, permohonan pemesanan darah Bapak/Ibu telah <strong>DISETUJUI</strong>.
  ...
  @elseif ($statusKey === 'rejected')
    <p class="note" style="margin-top:12px;">
      Berdasarkan verifikasi, permohonan pemesanan darah Bapak/Ibu <strong>BELUM DAPAT DIPENUHI</strong> saat ini karena keterbatasan stok.
  ...
  <div style="margin-top:24px; text-align:center; width:50%; margin-left:auto;">
    ...
    <!-- Cap overlay -->
    <img src="{{ public_path('images/cap-pmi.png') }}" class="stamp" alt="Cap PMI">
